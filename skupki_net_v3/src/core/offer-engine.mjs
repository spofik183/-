...
export function assessDealState(state = {}) {
  ...
  if (!state.details || String(state.details).trim().length < 12) reasons.push('details');
...
export function calculatePreliminaryOffer(state = {}) {
  ...
  let factor = 1;
  const positiveSignals = ['редк', 'легендар', 'лимит', 'топ', 'макс', 'высок', 'скин', 'инвентар', 'донат'];
  ...
  const low = Math.max(100, Math.round((center * 0.82) / 50) * 50);
  const high = Math.round((center * 1.18) / 50) * 50;
  return { low, high, currency: 'RUB', finalRequiresReview: true };
}
